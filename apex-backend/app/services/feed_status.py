...
async def set_feed_status(
    ...
    *,
    ...
    poll_received_at: datetime | None = None,
    age_seconds: int | None = None,
    consecutive_failures: int = 0,
    detail: str | None = None,
) -> None:
    poll_at = poll_received_at or datetime.now(timezone.utc)
    display_update = last_update or poll_at
    payload = {
        "symbol": symbol,
        "status": status.value,
        "status_ar": STATUS_AR[status.value],
        "last_update": display_update.isoformat(),
        "poll_received_at": poll_at.isoformat(),
...
async def get_all_feed_statuses(symbols: list[str]) -> dict[str, dict[str, Any]]:
    ...
    for sym in symbols:
        ...
        if data:
            poll_ts_raw = data.get("poll_received_at") or data.get("updated_at")
            if poll_ts_raw:
                try:
                    data["age_seconds"] = compute_age_seconds(
                        parse_utc_timestamp(str(poll_ts_raw)), now
                    )
                except (TypeError, ValueError):
                    pass
            elif data.get("last_update"):
                try:
