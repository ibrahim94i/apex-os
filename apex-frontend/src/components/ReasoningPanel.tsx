...
import type { AgentConsensus, AgentVerdict } from "@/types";
    ...
    <div className="agent-card">
      ...
      >
        <div className="agent-card-title">
          ...
          <span className="mono agent-confidence">
      ...
      {open && (
        <div className="agent-card-body">
          ...
          {verdict.error && (
            <div className="agent-error">{verdict.error}</div>
          )}
        </div>
      )}
    </div>
...
export default function ReasoningPanel({ consensus }: Props) {
  ...
  return (
    <div className="card col-12">
      ...
      <div className="consensus-summary">
        ...
        <span className={`signal-direction signal-${consensus.final_direction}`}>
          {translateDirection(consensus.final_direction)}
        </span>
        <span className="mono metric-value" style={{ marginRight: "0.5rem" }}>
          {(consensus.final_confidence * 100).toFixed(1)}%
        </span>
      </div>
      {consensus.reasoning_summary.length > 0 && (
        <div className="consensus-breakdown">
          {consensus.reasoning_summary.map((line, idx) => (
            <div key={idx} className="consensus-line">{line}</div>
          ))}
        </div>
      )}
